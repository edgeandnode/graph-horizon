...
import { config } from "../src/config"  // Use default config (same as handler uses)
import { DelegationPool, ServiceProvider, GraphNetwork } from "../generated/schema"
...
function setupDelegationPool(
  ...
  let verifier = config.subgraphServiceAddress
  ...
  pool.verifier = Bytes.fromHexString(verifier.toHexString())
  pool.tokens = tokens
